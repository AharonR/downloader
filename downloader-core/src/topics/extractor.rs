...
pub fn extract_keywords(text: &str) -> Vec<String> {
    TopicExtractor::new().extract(text)
}
...
mod tests {
    ...
    fn test_topic_extractor_creates_successfully() {
        let _extractor = TopicExtractor::new();
        // TopicExtractor::new() is infallible — construction succeeds by definition
    }
    ...
    fn test_extract_keywords_convenience_function() {
        let keywords = extract_keywords("Artificial Intelligence in Healthcare");
    ...
    fn test_extract_keywords_empty_returns_empty() {
        ...
        assert!(keywords.is_empty(), "Empty input should yield no keywords");
    }
