...
impl Queue {
    ...
    pub async fn enqueue(
        ...
        self.enqueue_with_metadata_in_project(url, source_type, original_input, None, None)
            .await
    }

    /// Adds a new item to the queue with pending status under an optional project scope.
    ///
    /// # Errors
    ///
    /// Returns [`QueueError::Database`] if the insert fails.
    #[instrument(skip(self), fields(url = %url, source_type = %source_type, project = project.unwrap_or("<none>")))]
    pub async fn enqueue_in_project(
        &self,
        url: &str,
        source_type: &str,
        original_input: Option<&str>,
        ...
        self.enqueue_with_metadata_in_project(url, source_type, original_input, None, project)
            .await
    }
    ...
    pub async fn enqueue_with_metadata(
        ...
        self.enqueue_with_metadata_in_project(url, source_type, original_input, metadata, None)
            .await
    }

    /// Adds a new item to the queue with optional metadata under an optional project scope.
    ///
    /// # Errors
    ///
    /// Returns [`QueueError::Database`] if the insert fails.
    #[instrument(skip(self, metadata), fields(url = %url, source_type = %source_type, project = project.unwrap_or("<none>")))]
    pub async fn enqueue_with_metadata_in_project(
        &self,
        url: &str,
        source_type: &str,
        original_input: Option<&str>,
    ...
    pub async fn dequeue(&self) -> Result<Option<QueueItem>> {
        self.dequeue_in_project(None).await
    }

    /// Retrieves and claims the next pending item for processing under an optional project scope.
    ///
    /// # Errors
    ///
    /// Returns [`QueueError::Database`] if the query fails.
    #[instrument(skip(self), fields(project = project.unwrap_or("<none>")))]
    pub async fn dequeue_in_project(&self, project: Option<&str>) -> Result<Option<QueueItem>> {
    ...
    pub async fn count_by_status(&self, status: QueueStatus) -> Result<i64> {
        self.count_by_status_in_project(status, None).await
    }

    /// Counts items by status under an optional project scope.
    ///
    /// # Errors
    ///
    /// Returns [`QueueError::Database`] if the query fails.
    #[instrument(skip(self), fields(project = project.unwrap_or("<none>"), status = %status))]
    pub async fn count_by_status_in_project(
        &self,
        status: QueueStatus,
        project: Option<&str>,
    ) -> Result<i64> {
    ...
    pub async fn has_active_url(&self, url: &str) -> Result<bool> {
        self.has_active_url_in_project(url, None).await
    }

    /// Checks whether a URL already has a pending or in-progress queue entry
    /// under an optional project scope.
    ///
    /// # Errors
    ///
    /// Returns [`QueueError::Database`] if the query fails.
    #[instrument(skip(self), fields(url = %url, project = project.unwrap_or("<none>")))]
    pub async fn has_active_url_in_project(
        &self,
        url: &str,
        project: Option<&str>,
    ) -> Result<bool> {
    ...
    pub async fn get_in_progress(&self) -> Result<Vec<QueueItem>> {
        self.get_in_progress_in_project(None).await
    }

    /// Returns all items currently in progress under an optional project scope.
    ///
    /// # Errors
    ///
    /// Returns [`QueueError::Database`] if the query fails.
    #[instrument(skip(self), fields(project = project.unwrap_or("<none>")))]
    pub async fn get_in_progress_in_project(
        &self,
        project: Option<&str>,
    ...
    pub async fn reset_in_progress(&self) -> Result<u64> {
        self.reset_in_progress_in_project(None).await
    }

    /// Resets in-progress rows back to pending status under an optional project scope.
    ///
    /// # Errors
    ///
    /// Returns [`QueueError::Database`] if the update fails.
    #[instrument(skip(self), fields(project = project.unwrap_or("<none>")))]
    pub async fn reset_in_progress_in_project(&self, project: Option<&str>) -> Result<u64> {
        let result = sqlx::query(
            r"UPDATE queue
              SET status = ?, updated_at = datetime('now')
              WHERE status = ?
    ...
    pub async fn list_by_status(&self, status: QueueStatus) -> Result<Vec<QueueItem>> {
        self.list_by_status_in_project(status, None).await
    }

    /// Lists items filtered by status under an optional project scope.
    ///
    /// Returns items ordered by priority (descending) and creation time (ascending).
    ///
    /// # Errors
    ...
    /// Returns [`QueueError::Database`] if the query fails.
    #[instrument(skip(self), fields(project = project.unwrap_or("<none>"), status = %status))]
    pub async fn list_by_status_in_project(
        &self,
        status: QueueStatus,
        project: Option<&str>,
    ) -> Result<Vec<QueueItem>> {
    ...
    /// Returns the number of legacy queue rows without project scope.
    ///
    /// This is used for diagnostics when transitioning from unscoped queue state.
    ///
    /// # Errors
    ///
    /// Returns [`QueueError::Database`] if the query fails.
    #[instrument(skip(self))]
    pub async fn count_legacy_unscoped_rows(&self) -> Result<i64> {
        let result = sqlx::query(r"SELECT COUNT(*) as count FROM queue WHERE project IS NULL")
            .fetch_one(self.db.pool())
            .await?;
        Ok(result.get("count"))
    }
